...
def load_model():
    """
    Loads the Dia TTS model into memory, storing model files in a persistent Modal volume path.
    """
    ...
    cache_path = get_model_cache_path()  # This should point to the Modal volume mount (e.g. /model_cache)
    ...
    try:
        # Directly download using hf_hub_download (it returns the full path)
        logger.info("Downloading config file to volume...")
        ...
        logger.info("Downloading weights file to volume...")
        ...
        if config is None:
            raise FileNotFoundError(f"Could not load config from {local_config_path}")
        ...
        model_config_instance = config
        EXPECTED_SAMPLE_RATE = DEFAULT_SAMPLE_RATE
        MODEL_LOADED = True

        logger.info("✅ Dia model loaded successfully from Modal volume.")
        return True

    except Exception as e:
        logger.error(f"❌ Failed to load Dia model: {e}", exc_info=True)
        dia_model = None
        MODEL_LOADED = False
        return False
